...
public class OrderMapper {
	...
	public DatabaseOrder mapToDatabaseOrder(DiscogsOrder order) throws IOException, InterruptedException {
		...
		if (statusDS.equals("Payment Pending") || statusDS.equals("Invoice Sent") || statusDS.equals("Payment Received")
				|| statusDS.equals("In Progress") || statusDS.equals("Shipped")
				|| statusDS.equals("Cancelled (Non-Paying Buyer)") || statusDS.equals("Cancelled (Item Unavailable)")
		...
		List<DatabaseOrder.DatabaseItem> dbItems = order.getItems().stream().map(item -> {
			...
			if (releaseService.contains(item.getRelease().getId())) {
				...
				DatabaseRelease temp = releaseService.getRelease(item.getRelease().getId());
				if (!temp.isArchived()) {
					release = temp;
				}
			}
			dbItem.setRelease(new DatabaseReleaseItem(release != null ? release.getId() : null,
					release != null ? release.getTitle() : "Undefined",
					release != null ? release.getArtists() : new ArrayList<Artist>(),
					release != null ? release.getThumb() : "Undefined"
